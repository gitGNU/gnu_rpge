 ...
 (choice-proc
  (lambda (dialog)
    (let* ((data (get-dialog-data dialog))
      ...
      (destroy-text (car data))
      (cons #t (cons ((cdr pair) dialog) (car pair)))))))
			
(add-bordered-and-standard-dialog-type! 
 vertical-menu
 ;Data is (cursor-index . ((string text-index proc) ..))
 (next-proc			
  (lambda (dialog)
 ...
 (process-proc
  (lambda (type data font sprite-data window)
    (let* ((sizes (get-window-dimensions window))
      ...
      (let ((first #t))
	(cons 0 (map (lambda (s) (list (car s)
				       (let ((y text-y))
					 (set! text-y (+ text-y text-dy))
					 (apply make-text text-x y (car s) font (if first (begin (set! first #f) '(0 255 255)) '(255 255 255))))
				       (cdr s)))
		     data))))))
 (choice-proc
  (lambda (dialog)
